...
def test_available_brands(advisor):
    """Проверка списка доступных брендов"""
...
def test_exact_brand_match(advisor):
    """Проверка точного поиска бренда"""
...
def test_fuzzy_brand_match(advisor):
    """Проверка поиска бренда с опечаткой"""
    test_brand = advisor.products[0]["brand"]
    # Создаем опечатку в последнем символе
...
def test_fuzzy_brand_no_match(advisor):
    """Проверка случая, когда бренд не найден"""
...
def test_extract_filters_ram(mock_openai, advisor):
    """Проверка извлечения фильтра по RAM"""
...
def test_extract_filters_gpt(mock_openai, advisor):
    """Проверка извлечения фильтров через GPT"""
    mock_client = MagicMock()
    mock_openai.return_value = mock_client
    mock_client.chat.completions.create.return_value = MagicMock(
...
def test_extract_filters_in_stock(advisor):
    """Проверка извлечения фильтра наличия товара"""
...
def test_apply_filters_with_preference(advisor):
    """Проверка учета предпочтений по бренду"""
    test_brand = advisor.products[0]["brand"]
...
def test_compare_products_invalid(advisor):
    """Проверка обработки неверных номеров для сравнения"""
...
def test_set_preference(advisor):
    """Проверка установки предпочтения по бренду"""
    test_brand = advisor.products[0]["brand"]
...
def test_set_preference_invalid(advisor):
    """Проверка ошибки при неверном бренде"""
...
def test_recommendation_fallback(advisor):
    """Проверка fallback-режима рекомендаций"""
    # Сохраняем оригинальный клиент
    original_client = advisor.client

    try:
        # Создаем мок для клиента, который будет вызывать ошибку
        mock_client = MagicMock()
        ...
        # Проверяем, что в результате есть хотя бы один бренд из продуктов
...
def test_currency_display(mock_openai, advisor):
    """Проверка отображения цен в долларах"""
    mock_client = MagicMock()
    mock_openai.return_value = mock_client
